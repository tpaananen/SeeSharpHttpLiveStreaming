...
namespace SeeSharpHttpLiveStreaming.Tests.Playlist.Tags.Media.MediaSegment
{
    ...
    public class KeyTests
    {
        ...
        public void TestKeyIsCreated(EncryptionMethod encryptionMethod)
        {
            ...
            _key = (Key)TagFactory.Create(new PlaylistLine("#EXT-X-KEY", value), BaseUri, 5);
        ...
        {
            var value = GetLine(EncryptionMethod.Aes128);
            var exception = Assert.Throws<SerializationException>(() => TagFactory.Create(new PlaylistLine("#EXT-X-KEY", value), BaseUri, version));
